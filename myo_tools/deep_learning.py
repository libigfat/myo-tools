...
class DeepLearning():
    ...
    def model(self, input_size, output_size):
        x = tf.placeholder(tf.float32, shape=[None, input_size[0], input_size[1], input_size[2]], name='x')
        y = tf.placeholder(tf.float32, shape=[None, output_size])
        cnn1 = tf.layers.Conv2D(10, [3, 3], activation=tf.nn.relu)(x)
        pool1 = tf.layers.MaxPooling2D([2, 2], [2, 2])(cnn1)
        cnn2 = tf.layers.Conv2D(20, [3, 3], activation=tf.nn.relu)(pool1)
        pool2 = tf.layers.MaxPooling2D([2, 2], [2, 2])(cnn2)
        f = tf.layers.Flatten()(pool2)
        shadow = tf.layers.Dense(50, activation=tf.nn.relu)(f)
        output = tf.layers.Dense(output_size)(shadow)
        return x, y, output
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.num_epochs):
                ...
                if (i + 1) % 10 == 0:
                    ...
                    saver.save(sess, save_path + 'cnn', global_step=i + 1)
                    print("保存参数")
